...
# Anchor points for Tusimple and Culane datasets
tusimple_row_anchor = [
...
class UltrafastLaneDetector:
    ...
    def initialize_model(model_path, cfg, use_gpu):
        ...
        net = parsingNet(
            ...
            use_aux=False  # We don't need auxiliary segmentation in testing
        )
    ...
    def initialize_image_transform():
        # Create transformation to resize and normalize the input images
    ...
    def process_output(output, cfg):
        ...
        # Debugging: Print shapes of lanes_points and lanes_detected
        print("lanes_points before conversion:", lanes_points)
        print("lanes_detected before conversion:", lanes_detected)

        # Ensure consistent shapes
        max_points = max(len(points) for points in lanes_points)
        lanes_points = [points + [[-1, -1]] * (max_points - len(points)) for points in lanes_points]
    ...
    def draw_lanes(input_img, lanes_points, lanes_detected, cfg, draw_points=True):
        # Write the detected lane points on the image
